...
class Module:
    ...
    def __evaluateTransitBuffers(self):
        import psycopg2
        ...
        try:
            ...
            while error and count < 3:
                ...
                try:
                    query = """
                        select urbper_buffer_transit({scenario})
                            """.format(scenario=self.__scenario)
                    LogEvents(
                        "transit buffers",
                        "transit buffer module started: " + query,
                        self.__scenario,
                        self.__user
                    )                    
                    ...
                    LogEvents(
                        "transit buffers",
                        "transit buffers module finished",
                        self.__scenario,
                        self.__user
                    )
        except Exception as e:
