...
class Module:
    ...
    def run(self):
        try:
            ...
            sea_classes=self.__getClassess("sea")
            
            sea_array="'{"+",".join(sea_classes)+"}'"
            ...
            while error and count < 3:
                ...
                try:
                    query = """
                        select urbper_indicator_population_in_sea_floods({scenario},{sea})
                        """.format(
                            scenario=self.__scenario,
                            sea=sea_array
                        )
                    LogEvents(
                        "urbper_indicator population in sea floods",
                        "population in sea floods started: " + query,
                        self.__scenario,
                        self.__user
                    )
                    ...
                    db.close()
                    LogEvents(
                        "population in sea floods",
                        "population in sea floods module failed " +
                        str(count) + ": " + str(e),
                        self.__scenario,
                        self.__user
                    )
                    ...
                    LogEvents(
                        "population in sea floods",
                        "population in sea floods module finished",
                        self.__scenario,
                        self.__user
                    )
        except Exception as e:
