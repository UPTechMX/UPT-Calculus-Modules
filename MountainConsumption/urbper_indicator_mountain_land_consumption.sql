 ...
 */
CREATE OR REPLACE FUNCTION urbper_indicator_mountain_land_consumption (scenario_par INT, result_name text, mountain TEXT[])
  RETURNS void
  LANGUAGE 'plpgsql'
  VOLATILE
  AS $$
...
BEGIN
  ...
  ,mmu_scen as(
    ...
    from 
      ...
      ,mountain_polygons,base_footprint
    where 
      mmu.scenario_id = scenario_par
      and st_contains(mountain_polygons.geom, mmu.location)
      and not st_contains(base_footprint.geom, mmu.location)
  )
  -- get mmu area within mountain polygons
  SELECT
