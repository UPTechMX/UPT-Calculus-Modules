    ...
    def run(self):
        try:
            ...
            while error and count < 3:
                ...
                try:
                    query = """
                        select urbper_indicator_water_consumption({scenario})
                            """.format(scenario=self.__scenario)
                    LogEvents(
                        "water consumption",
                        "water consumption module started: " + query,
                        self.__scenario,
                        self.__user
                    )
                    ...
                    LogEvents(
                        "water consumption",
                        "water consumption module failed " +
                        str(count) + ": " + str(e) ,
                        self.__scenario,
                        self.__user
                    )
                    ...
                    LogEvents(
                        "water consumption",
                        "water consumption module finished",
                        self.__scenario,
                        self.__user
                    )
        except Exception as e:
