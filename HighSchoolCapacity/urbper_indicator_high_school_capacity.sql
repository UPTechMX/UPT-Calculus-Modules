 ...
 */
CREATE OR REPLACE FUNCTION urbper_indicator_high_school_capacity (scenario_par INT, result_name text, amenity_list TEXT[])
  RETURNS void
  LANGUAGE 'plpgsql'
  VOLATILE
  AS $$
DECLARE
  highstudent_area double precision;
  total double precision;
BEGIN
  SELECT
    value INTO highstudent_area
  FROM
    assumptions
  WHERE
    scenario_id = scenario_par
    AND assumptions.category = 'criteria'
    AND assumptions.name = 'highstudent_area';
  ...
  SELECT
    sum(c1.value * c2.value / highstudent_area) into total
  FROM (
    SELECT
