 ...
 */
CREATE OR REPLACE FUNCTION urbper_indicator_educational_infrastructure_in_risk(scenario_par INT, result_name text, amenity_list TEXT[], risk_list TEXT[])
  RETURNS void
  LANGUAGE 'plpgsql'
  VOLATILE
  AS $$
...
BEGIN
  ...
  FROM
    ...
    risk_polygons
  WHERE
    amenities.fclass= ANY (amenity_list)
    and scenario_id = scenario_par
