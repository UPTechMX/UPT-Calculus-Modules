            ...
            while error and count < 3:
                ...
                try:
                    with transaction.atomic():
                        query = """select urbper_indicator_mmu_transit_distance({scenario},{offset},{limit})""".format(
                            scenario=scenario_id, offset=offset, limit=limit)
                        LogEvents("mmu transit distance", "mmu transit distance  module started: " + query,
                    ...
                    error = True
                    count += 1
                    LogEvents("mmu transit distance ", "mmu transit distance  module failed " +
                              str(count) + ": " + str(e), scenario_id, self.__user, True)
                    db.close()
                else:
                    error = False
                    
                    LogEvents("mmu transit distance ", "mmu transit distance  module finished",
                              scenario_id, self.__user)
                    db.close()
        except Exception as e:
            LogEvents("Running mmu transit distance", "Unknown error " +
