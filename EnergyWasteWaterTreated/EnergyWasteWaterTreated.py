    ...
    def run(self):
        try:
            ...
            while error and count < 3:
                ...
                try:
                    query = """
                        select urbper_indicator_energy_waste_water_treated({scenario})
                            """.format(scenario=self.__scenario)
                    LogEvents(
                        "energy waste water treated",
                        "energy waste water treated module started: " + query,
                        self.__scenario,
                        self.__user
                    )
                    ...
                    LogEvents(
                        "energy waste water treated",
                        "energy waste water treated module failed " +
                        str(count) + ": " + str(e) ,
                        self.__scenario,
                        self.__user
                    )
                    ...
                    LogEvents(
                        "energy waste water treated",
                        "energy waste water treated module finished",
                        self.__scenario,
                        self.__user
                    )
        except Exception as e:
