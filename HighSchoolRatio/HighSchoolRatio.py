    ...
    def run(self):
        try:
            ...
            while error and count < 3:
                ...
                try:
                    query = """
                        select urbper_indicator_high_school_ratio({scenario},'highmax_perc'::varchar(30))
                            """.format(scenario=self.__scenario)
                    LogEvents(
                        "high school ratio",
                        "high school ratio module started: " + query,
                        self.__scenario,
                        self.__user
                    )
                    ...
                    LogEvents(
                        "high school ratio",
                        "high school ratio module failed " +
                        str(count) + ": " + str(e) ,
                        self.__scenario,
                        self.__user
                    )
                    ...
                    LogEvents(
                        "high school ratio",
                        "high school ratio module finished",
                        self.__scenario,
                        self.__user
                    )
        except Exception as e:
