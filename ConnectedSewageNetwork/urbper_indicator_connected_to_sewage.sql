 ...
 */
CREATE OR REPLACE FUNCTION urbper_indicator_connected_to_sewage (scenario_par INT)
  RETURNS void
  LANGUAGE 'plpgsql'
  VOLATILE
  AS $$
...
BEGIN
  ...
  where 
    ...
    VALUES (scenario_par, 'pop_con_sew', total) ON CONFLICT (scenario_id, name)
    DO
    UPDATE
    SET
      VALUE = total;
    ...
    INSERT INTO results (scenario_id, name, value)
    VALUES (scenario_par, 'con_sew', total / total_pop * 100) ON CONFLICT (scenario_id, name)
    DO
    UPDATE
    SET
