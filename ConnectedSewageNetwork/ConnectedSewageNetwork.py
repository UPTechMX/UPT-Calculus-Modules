    ...
    def run(self):
        try:
            ...
            while error and count < 3:
                ...
                try:
                    query = """
                        select urbper_indicator_connected_to_sewage({scenario})
                        """.format(
                            scenario=self.__scenario
                        )
                    LogEvents(
                        "connected to sewage",
                        "connected to sewage module started: " + query,
                        self.__scenario,
                        self.__user
                    )
                    ...
                    db.close()
                    LogEvents(
                        "connected to sewage risk",
                        "connected to sewage module failed " +
                        str(count) + ": " + str(e),
                        self.__scenario,
                        self.__user
                    )
                    ...
                    LogEvents(
                        "connected to sewage",
                        "connected to sewage module finished",
                        self.__scenario,
                        self.__user
                    )
        except Exception as e:
