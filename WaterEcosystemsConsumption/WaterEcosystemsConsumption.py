...
class Module:
    ...
    def run(self):
        try:
            ...
            water_eco_classes=self.__getClassess("water_eco")
            
            water_eco_array="'{"+",".join(water_eco_classes)+"}'"
            ...
            while error and count < 3:
                ...
                try:
                    query = """
                        select urbper_indicator_water_ecosystems_consumption({scenario},'watereco_consumption',{water_eco})
                        """.format(
                            scenario=self.__scenario, 
                            water_eco=water_eco_array,
                        )
                    LogEvents(
                        "water ecosystems consumption",
                        "water ecosystems consumption module started: " + query,
                        self.__scenario,
                        self.__user
                    )
                    ...
                    LogEvents(
                        "water ecosystems consumption",
                        "water ecosystems consumption module failed " +
                        str(count) + ": " + str(e),
                        self.__scenario,
                        self.__user
                    )
                    ...
                    LogEvents(
                        "water ecosystems consumption",
                        "water ecosystems consumption module finished",
                        self.__scenario,
                        self.__user
                    )
        except Exception as e:
