...
class Module:
    ...
    def __evaluateRiskBuffers(self):
        import psycopg2
        ...
        try:
            ...
            while error and count < 3:
                ...
                try:
                    query = """
                        select urbper_buffer_risk({scenario})
                            """.format(scenario=self.__scenario)
                    LogEvents(
                        "risk buffers",
                        "risk buffer module started: " + query,
                        self.__scenario,
                        self.__user
                    )
