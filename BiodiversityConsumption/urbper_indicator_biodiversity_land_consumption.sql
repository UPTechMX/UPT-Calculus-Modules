...
This function counts the number of facilities located in risk areas
 ...
 biodiversity:text[] array with the class of the risk to be used as risk areas
 */
CREATE OR REPLACE FUNCTION urbper_indicator_biodiversity_land_consumption (scenario_par INT, result_name text, biodiversity TEXT[])
  RETURNS void
  LANGUAGE 'plpgsql'
  VOLATILE
  AS $$
...
BEGIN
  ...
  WITH biodiversity_polygons AS (
      SELECT
        st_union (LOCATION) AS geom
      FROM
        footprint
      where 
        footprint.name= ANY (biodiversity)
  ...
  ,mmu_scen as(
    ...
    --and mmu_info.name = 'area'
      ,biodiversity_polygons,base_footprint
    where 
      mmu.scenario_id = scenario_par
      and st_contains(biodiversity_polygons.geom, mmu.location)
      and not st_contains(base_footprint.geom, mmu.location)
  )
  -- get mmu area within biodiversity polygons
  SELECT
