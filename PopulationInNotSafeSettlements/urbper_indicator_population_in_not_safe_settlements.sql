 ...
 */
CREATE OR REPLACE FUNCTION urbper_indicator_population_in_not_safe_settlements (scenario_par INT,not_safe_list TEXT[])
  RETURNS void
  LANGUAGE 'plpgsql'
  VOLATILE
  AS $$
...
BEGIN
  ...
  WITH slum_polygons AS (
    ...
    WHERE
      scenario_id = scenario_par
      AND fclass = ANY (not_safe_list)
  )
  --sum population inside not_safe
  SELECT
  ...
  VALUES (scenario_par, 'pop_notsafe', total) ON CONFLICT (scenario_id, name)
  DO
    UPDATE
  SET
    VALUE = total;
  ...
  INSERT INTO results (scenario_id, name, value)
  VALUES (scenario_par, 'perc_notsafe', total / total_pop * 100) ON CONFLICT (scenario_id, name)
  DO
    UPDATE
  SET
