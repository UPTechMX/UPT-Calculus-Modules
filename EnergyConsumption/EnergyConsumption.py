    ...
    def run(self):
        try:
            ...
            while error and count < 3:
                ...
                try:
                    query = """
                        select urbper_indicator_energy_consumption({scenario},'energy_consumption')
                            """.format(scenario=self.__scenario)
                    LogEvents(
                        "energy consumption",
                        "energy consumption module started: " + query,
                        self.__scenario,
                        self.__user
                    )
                    ...
                    LogEvents(
                        "energy consumption",
                        "energy consumption module failed " +
                        str(count) + ": " + str(e) ,
                        self.__scenario,
                        self.__user
                    )
                    ...
                    LogEvents(
                        "energy consumption",
                        "energy consumption module finished",
                        self.__scenario,
                        self.__user
                    )
        except Exception as e:
